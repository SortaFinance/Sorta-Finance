...
mod test {
    ...
    fn test_farm_raydium() {
        ...
        assert!(farm.eq(&Farm::Raydium {
            name: Raydium::SORTAUSDC
        }));
        assert!(farm.name().eq(&"RAYDIUM-SORTA-USDC".to_string()));
        assert!(Farm::from(Raydium::from("SORTA-USDC")).eq(&Farm::Raydium {
            name: Raydium::SORTAUSDC
        }));
    ...
    fn test_farm_from() {
        ...
        assert!(Farm::from([0_u64, 17_u64]).name() == *"RAYDIUM-SORTA-USDC");

        assert!(Farm::from([1_u64, 0_u64]).name() == *"LENDING-SORTA");
    ...
    fn test_farm_lending() {
        ...
        assert!(farm.name().eq(&"LENDING-SORTA".to_string()));
