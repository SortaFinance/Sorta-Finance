...
impl Raydium {
    pub fn name(&self) -> String {
        match self {
            ...
            Raydium::SORTAUSDC => String::from("SORTA-USDC"),
...
mod test {
    ...
    fn raydium() {
        ...
        assert!(farm == Raydium::SORTAUSDC);
        assert!(farm.name().eq(&"SORTA-USDC".to_string()));
        assert_eq!(farm.dual_reward(), false);
        assert!(Raydium::from("SORTA-USDC") == Raydium::SORTAUSDC);
        let number: u64 = farm.into();
        assert_eq!(number, 17);
